...
func setupManagerTest[T any](t *testing.T, content string) (*Manager, *T) {
	...
	m, err := NewManager(t.Context(), reg, providerReg, cfg)
...
`
	...
	m, cfg := setupManagerTest[testConfig](t, content)
...
`
	...
	require.NoError(t, m.HydrateConfig(t.Context(), cfg))
...
`
	...
	require.Eventuallyf(t, func() bool {
		cfg = parseConfig[testConfig](t, content)
		require.NoError(t, m.HydrateConfig(t.Context(), cfg))
		return cfg.APIKeys[0].Value() == "recovered_secret"
...
api_keys:
  - "%s"
`, inlineSecret)
	_, cfg := setupManagerTest[testConfig](t, content)
