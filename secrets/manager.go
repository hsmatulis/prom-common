...
const (
	...
	// defaultHydrateFetchTimeout governs the default maximum time a fetch can block during .Hydrate() calls.
	defaultHydrateFetchTimeout = 0 * time.Millisecond
...
func NewManager(logger *slog.Logger, r prometheus.Registerer, providers *ProviderRegistry, config interface{}) (*Manager, error) {
	...
	manager := &Manager{
		...
		logger:   logger,
		// blocks waiting for up to newFetchTimeout on the first hydration.
		registerFetchTimeout: newFetchTimeout,
		providers:            providers,
	}
	promslog.NewNopLogger()
	manager.registerMetrics(r)
	if err := manager.HydrateConfig(config); err != nil {
		return nil, err
	}
	manager.registerFetchTimeout = defaultHydrateFetchTimeout
	return manager, nil
}
...
func (m *Manager) registerSecret(path string, s *Field) error {
	...
	mSecret, exists := m.secrets[state.id()]
	if !exists {
...
func (m *Manager) HydrateConfig(config interface{}) error {
	...
	if err := runPreppers(m, config); err != nil {
		return err
	}
	...
	for field, path := range fields.paths {
		...
		} else if _, ok := droppedSecrets[field.state.id()]; ok {
			delete(droppedSecrets, field.state.id())
		}
	}
...
func (m *Manager) triggerRefresh(s *Field) {
	m.mtx.RLock()
	defer m.mtx.RUnlock()
	ms, ok := m.secrets[s.state.id()]

	if !ok {
		m.logger.Warn("triggerRefresh: secret with not found; refresh not triggered", "secret_id", s.state.id())
		return
	}
