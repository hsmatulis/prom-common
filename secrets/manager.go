...
func NewManager(ctx context.Context, r prometheus.Registerer, providers *ProviderRegistry, config interface{}) (*Manager, error) {
	...
	if err := manager.HydrateConfig(ctx, config); err != nil {
		return nil, err
	}
	return manager, nil
}
...
func (m *Manager) registerSecret(ctx context.Context, path string, s *Field) error {
	if s.state != nil {
...
func (m *Manager) HydrateConfig(ctx context.Context, config interface{}) error {
	m.mtx.Lock()
	m.hydrating = true
	defer func() {
		m.hydrating = false
	...
	for field, path := range fields.paths {
		if err := m.registerSecret(ctx, path, field); err != nil {
